...
case $INPUT_RULE_SET in
  ...
  "FedRAMP-Moderate")
    ...
    sh -c "cfn-guard validate --show-summary ${INPUT_SHOW_SUMMARY} --output-format ${INPUT_OUTPUT_FORMAT} --data ${INPUT_DATA_DIRECTORY} --rules /${INPUT_RULE_SET}.guard; echo 'scan complete'"
    echo -n "cfn-guard scan complete for ${INPUT_RULE_SET}"
    ;;
